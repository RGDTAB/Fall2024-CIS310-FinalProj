...
class ThermostatPainter extends CustomPainter {
  ...
  void paintGlass(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
        Offset(centerX, centerCircleY), _circleRadiusGlass, _getPaintGlass());

    final startRectX = centerX - (_rectGlassWidth / 2);
    final tl = size.topLeft(Offset(startRectX, 0));

    final rect = Rect.fromLTWH(
        tl.dx, tl.dy, _rectGlassWidth, size.height - _circleRadiusGlass);

    final radiusX = _rectGlassWidth / 2;
    final radiusY = _rectGlassWidth / 2;

    canvas.drawRRect(
        RRect.fromRectXY(rect, radiusX, radiusY), _getPaintGlass());
  }
  ...
  void paintGlassFill(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
        Offset(centerX, centerCircleY), circleRadius, _getPaintGlassFill());
  ...
  void paintScale(Canvas canvas, Size size) {
    for (var i = scaleLow; i <= scaleHigh; i += scaleStep) {
      final y = map(i - scaleLow, 0, _scaleRange,
          size.height - (_circleRadiusGlass * 2), 0 + (_rectGlassWidth / 2));

      final xRectRight = (size.width / 2) + (_rectGlassWidth / 2);

      canvas.drawLine(Offset(xRectRight, y),
          Offset(xRectRight + _scaleStrokeWidth, y), _getPaintScale());
      ...
      textPainter.paint(
          canvas,
          Offset(xRectRight + _scaleStrokeWidth - textPainter.width,
